...
#' @export
#'
capacity_climate_reg_scot <- function(x = parent.frame()$mm,
  ...
  x <- x[x$HabClass %in% c(
    "Woodland and scrub",
    "Water", "Green urban surfaces"
  ) | x$HabCode_B == "J21", ] # make sure hedgerows can contribute to service
  ...
  b1 <- dplyr::filter(x, area <= 20000) %>%
    sf::st_buffer(20) %>%
    sf::st_union() %>%
    sf::st_sf()

  b2 <- dplyr::filter(x, dplyr::between(area, 20000, 50000)) %>%
    sf::st_buffer(40) %>%
    sf::st_union() %>%
    sf::st_sf()

  b3 <- dplyr::filter(x, dplyr::between(area, 50000, 100000)) %>%
    sf::st_buffer(80) %>%
    sf::st_union() %>%
    sf::st_sf()

  b4 <- dplyr::filter(x, area > 100000) %>%
    sf::st_buffer(100) %>%
    sf::st_union() %>%
    sf::st_sf()
  ...
  clim_score <- mask(clim_score, rast(mask_r), # apply the mask on the raster object
    filename = file.path(scratch, "climreg_score2.tif"),
    overwrite = TRUE
  )
  ...
  clim_score <- classify(clim_score,
    cbind(NA, 0), # change the NAs
    filename = file.path(scratch, "climreg_score.tif"),
    overwrite = TRUE
  )
