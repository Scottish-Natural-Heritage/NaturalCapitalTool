...
# 2024 update: added popmask as in the access to nature demand model; scaled health and population indicators to 0-1 scales
...
#' @return A raster with demand scores, rescaled from 0 to 3 (where 3 is maximum demand for noise regulation).
#' @export
#'
demand_noise_scot <- function(x = parent.frame()$mm,
  ...
  SAbuffer <- sf::st_buffer(sf::st_geometry(studyArea), 500) %>%
    sf::st_as_sf() %>%
  ...
  if (indic_weights[1] > 0) { # only if user wants this indicator
    ...
    dist_motor <- if (raster::hasValues(dist_motor)) {
      1 - (log10(dist_motor + 1) / log10(distances[["motorway"]]))
    } else {
      r
    }
    dist_roadsA <- if (raster::hasValues(dist_roadsA)) {
      1 - (log10(dist_roadsA + 1) / log10(distances[["primary"]]))
    } else {
      r
    }
    dist_dualcarriageways <- if (raster::hasValues(dist_dualcarriageways)) {
      1 - (log10(dist_dualcarriageways + 1) / log10(distances[["dual"]]))
    } else {
      r
    }
    dist_rail <- if (raster::hasValues(dist_rail)) {
      1 - (log10(dist_rail + 1) / log10(distances[["railway"]]))
    } else {
      r
    }
    dist_air <- if (raster::hasValues(dist_air)) {
      1 - (log10(dist_air + 1) / log10(distances[["airport"]]))
    } else {
      r
    }
    ...
    if (terra::hasValues(distances)) {
      ...
      if (raster::nlayers(distances) > 1){
       ...
       distancescore <- raster::unstack(distances) 
       distancescore <- raster::brick(distancescore)
    ...
    # save raw indicator
    if (indicators) {
      raster::writeRaster(distancescore,
        filename = file.path(
          indicator_path,
  ...
  if (indic_weights[2] > 0) { # only if user wants this indicator
    ...
    popscore <- raster::rasterize(houses, r,
  ...
  # Make a copy of popscore for masking
  ...
  if (indic_weights[3] > 0) { # only if user wants this indicator
    ...
    healthscore <- raster::rasterize(houses, r,
  ...
  # popscore is square root transformed based on a minimum value of pop_density (min population to be considered as having demand
  ...
  pop_density <- pop_threshold * (pi * local^2 / 10000)

  if(hasValues(popscore)==TRUE){
    popscore <- rescale_score(popscore, minval = pop_density, score_type = "pop")
  }else{
    popscore[is.na(popscore)] <- 0
    popscore <- rast(popscore)
  }
  
  if(hasValues(healthscore)==TRUE){
    healthscore <- rescale_score(healthscore, score_type = "health")
  }else{
    healthscore[is.na(healthscore)] <- 0
    healthscore <- rast(healthscore)
  }
  ...
  final <- sum(
    indic_weights[1] * rast(distancescore),
  ...
  ## Mask the raster
  if(hasValues(pop_mask)){
    pop_mask[pop_mask <= pop_density] <- NA
    
    final <- mask(final, pop_mask)
    
    
  }

  
  rm(distancescore, popscore, pop_mask, healthscore)
  ...
  final <- raster::writeRaster(
    raster::mask(final, studyArea),
    filename = file.path(save,
                         paste0(projectLog$title, "_", runtitle, "_noise_reg_demand.tif")),
