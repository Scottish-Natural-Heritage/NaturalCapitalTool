...
#' @param custom_ext A custom extent; the final output will
...
#' @export
#'
capacity_air_scot <- function(x = parent.frame()$mm,
  ...
  air_score <- writeRaster(sum(air_short, air_long, na.rm = TRUE),
    filename = file.path(scratch, "airpur_score.tif"),
    overwrite = TRUE
  )
  ...
  air_score <- mask(air_score, airmask)


  ### Create functional threshold mask ----
  message("Applying functional threshold.")
  
  # check for values, otherwise skip to final save
  if(!is.na(minmax(air_score)[1]) & !is.na(minmax(air_score)[2])){
