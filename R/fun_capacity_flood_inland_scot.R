...
#' @param spr File path to a raster file containing Standard Percentage Runoff values (optional). Default is to leave it out, but if specified it will be included in the scoring.
...
#' @return A raster with raw capacity scores (arbitrary units).
#' @export
#'
capacity_flood_inland_scot <- function(x = parent.frame()$mm,
  ...
  if (is.null(DTM)) {
    ...
    if (inherits(dtm, "try-error") | is.na(dtm)) {
      stop("No elevation dataset found in project log, and you did not specify argument DTM")
    }
  } else {
    dtm <- DTM

    if (!dir.exists(dtm)) {
      stop("No such folder: ", dtm)
    }
  }
  ...
  if (length(dtm) == 0) {
    stop("DTM doesn't intersect your study area.")
  } else if (length(dtm) == 1) {
  ...
  ## Now we should be working with one single dtm for the rest of the model
  ...
  # TO DO - check that spr is a raster OR basemap attribute?
  ## Check SPR
  ...
  # TO DO: allow for spr file path OR spr classes in basemap
  ...
  slopes <- raster::terrain(dtm, "slope", "tangent", # used to be degrees
  ...
  slopes <- raster::reclassify(slopes,
    ...
    include.lowest = TRUE,
    filename = file.path(scratch, "slopesrcl"), overwrite = TRUE
  ) # note: saving under new name, overwriting may cause unexpected behaviour
  ...
  rough_r <- rescale_indicator(rough_r)
  ...
  # Score is obtained by combining roughness & slope
  ...
  if (!is.null(spr)) {
    ...
    # Soil run off raster
    soil_r <- raster::raster(spr)
    soil_r <- crop(soil_r, r)

    # flip the values (high runoff means low permeability) and rescale to 0-1 (instead of 0-100)

    soil_r <- (100 - soil_r) / 100
    
    # Assign highest SPR value to areas with no SPR
    soil_r <- subst(rast(soil_r), NA, 0.05)
    soil_r <- raster::raster(soil_r)

    # align with raster template
    soil_r <- raster::resample(soil_r, r, "bilinear",
      filename = file.path(scratch, "spr"), overwrite = TRUE
    )
  ...
  watermask <- dplyr::filter(x, HabBroad %in% c("Water, fresh", "Water, sea", "Water, brackish"))
  ...
  final <- terra::writeRaster(
    terra::mask(water_score, vect(studyArea)),
