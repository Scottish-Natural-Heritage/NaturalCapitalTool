...
#' @export
#'
capacity_noise_scot <- function(x = parent.frame()$mm,
  ...
  noise_score <- writeRaster(
    sum(noise_short, noise_long, na.rm = TRUE),
    filename = file.path(scratch, "noisereg_score.tif"),
    overwrite = TRUE
  )
  ...
  if(!is.na(minmax(noise_score)[1]) & !is.na(minmax(noise_score)[2])){
  ...
  noise_score <- raster::writeRaster(
    functionalMask(noise_score,  # the score raster we apply the function to
                   ...
                   proportion = 0.10,  # proportion of cells in the search circle which can have a score of 10 without being considered to contribute to the service
