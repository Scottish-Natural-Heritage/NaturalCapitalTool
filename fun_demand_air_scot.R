##################################################################
### Air quality improvement demand model                       ###
### 12 October 2021                                            ###
...
#' Air Quality Improvement Demand Model
#'
#' Runs the air quality improvement ecosystem service model, generating demand scores based on four indicators: distance to major roads, proportion of manmade surfaces, population, and health. (Specific indicators can be omitted by setting the appropriate weight to 0.)
...
#' @param roadClassification The attribute name in OS Open Roads containing the classification of road types, default "road_classification". Only "Motorway" and "A Road" types are selected for analysis. (If using custom road network, please ensure classification matches these terms exactly.)
...
#' @return A raster with demand scores, rescaled from 0 to 4 (where 4 is maximum demand for air quality improvement).
#' @export
#'
demand_air_scot <- function(x = parent.frame()$mm,
  ...
  if (is.numeric(indic_weights)) {
    if (length(indic_weights) != 4) { 
      message("Weights should have 4 values for this model. Defaulting to uniform weights of 1.")
      indic_weights <- c(1, 1, 1, 1)
    }
  } else {
    stop("Weights must be a numeric vector of length 4 for this model. Please refer to documentation.")
  }
  ...
  if (any(!attributes %in% names(x))) {
    stop(
      "Could not find attribute(s) in your basemap: ",
  ...
  if (indic_weights[1] > 0) { # only if user wants this indicator
    ...
    ## TODO: improve handling in cases of multilinestring etc
    ...
    roadscore <- distance_from_source(mmroads, raster::raster(r), "air_demand_roads_score", local)
    ...
    roadscore <- terra::rast(roadscore)

    # save raw indicator
    if (indicators) {
      roadscore <- terra::writeRaster(roadscore,
        filename = file.path(
          indicator_path,
  ...
  if (indic_weights[2] > 0) { # only if user wants this indicator
    ...
    manmade <- fasterize::fasterize(manmade,
      raster::raster(r),
      field = NULL
    ) # manmade features get value of 1
    ...
    # TODO: currently omitted due to instability
    ...
    if (indicators) {
      terra::writeRaster(manmadescore,
        filename = file.path(
          indicator_path,
  ...
  if (indic_weights[3] > 0) { # only if user wants this indicator
    ...
    population <- raster::rasterize(houses, r,
      ...
      filename = file.path(scratch, "airpurif_pop_temp.tif"), overwrite = TRUE
    )
    ...
    # roadscore <- raster::mask(roadscore, roadmask)
  ...
  if (indic_weights[4] > 0) { # only if user wants this indicator
    ...
    healthmap <- raster::rasterize(houses, r,
      ...
      filename = file.path(scratch, "airpurif_health_temp.tif"),
      overwrite = TRUE
    )
  ...
  # popscore2 is square root transformed based on a minimum value of pop_density (min population to be considered as having demand
  # for the service) and maximum value of the maximum population within 300 m for all of Scotland (3741 as of 2011 census)
  #
  # healthscore is min/max transformed based on a minimum value of 0 and a maximum value of 360 (the maximum value for all of Scotland)
  ...
  healthscore <- rescale_score(healthscore, score_type = "health")
  ...
  rm(roadscore, manmadescore, popscore2, healthscore)

  ## Apply a population threshold

  ## Mask the raster

  popscore[popscore <= pop_density] <- NA

  final <- terra::mask(final, popscore)
  ...
  # Mask to study area
  crs(final) <- crs(studyArea)
  final <- terra::mask(final, studyArea)


  message("Saving final scores.")

 terra::writeRaster(final,
  ...
  on.exit({
    rm(r, final, maxvalue)
    cleanUp(scratch)
    message("Air quality improvement demand model finished. Process took ", round(difftime(timeB, timeA, units = "mins"), digits = 1), " minutes. Please check output folder for your maps.")
  })
